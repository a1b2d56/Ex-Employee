...
@HiltViewModel
class VerificationViewModel @Inject constructor(private val repo: EmployeeRepository) : ViewModel() {
    private val _items = MutableStateFlow<UiState<List<VerificationDoc>>>(UiState.Idle)
    val items: StateFlow<UiState<List<VerificationDoc>>> = _items
    fun load(token: String) = viewModelScope.launch { _items.value = UiState.Loading; _items.value = repo.getVerificationItems(token) }
}
