...
@HiltViewModel
class NoticeboardViewModel @Inject constructor(private val repo: EmployeeRepository) : ViewModel() {
    private val _notices = MutableStateFlow<UiState<List<Notice>>>(UiState.Idle)
    val notices: StateFlow<UiState<List<Notice>>> = _notices
    fun load(token: String) = viewModelScope.launch { _notices.value = UiState.Loading; _notices.value = repo.getNotices(token) }
}
