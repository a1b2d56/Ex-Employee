...
@HiltViewModel
class DependantsViewModel @Inject constructor(private val repo: EmployeeRepository) : ViewModel() {
    private val _items = MutableStateFlow<UiState<List<Dependant>>>(UiState.Idle)
    val items: StateFlow<UiState<List<Dependant>>> = _items
    fun load(token: String) = viewModelScope.launch { _items.value = UiState.Loading; _items.value = repo.getDependants(token) }
}
